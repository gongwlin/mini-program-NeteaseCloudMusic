  ...
  if (type == 1) {
    ...
    if (day==jm.getDate()) {
      ...
      if (Fo < Qq + 864e5) {
        ...
        let hq = jm.getFullYear()
          , bOg = new Date(hq, 0, 1);
        let Qq = jm.getTime() - bOg.getTime();
        if (Fo < Qq) {
 ...
 * @param {*} duration 
 * 格式化时间
 * 传入毫秒 返回分钟：秒格式的时间格式   60000 => 01:00
 */
...
function parse_lrc(lrc_content) {
  ...
  let scroll = true;
  for (let i in lrc_row) { // 遍历每一行
    if ((lrc_row[i].indexOf(']') == -1) && lrc_row[i]) { // 每行没有]
...
function playAlrc(that, app) {
  ...
  wx.getBackgroundAudioPlayerState({
    complete: function(res){
      let time = 0, lrcindex = that.data.lrcindex, playing = false, playtime = 0, downloadPercent = 0;
      if (res.status != 2) {
        time = res.currentPosition / res.duration * 100;
        playtime = res.currentPosition;
        downloadPercent = res.downloadPercent || 0
        if (that.data.showlrc && !that.data.lrc.scroll) {
          for (let i in that.data.lrc.lrc) {
            let se = that.data.lrc.lrc[i];
...
function loadlrc(that) {
  ...
  if (!that.data.lrc.code) {
    let lrcid = that.data.music.id;
    let that = that;
    // wx.request({
    //   url: bsurl + 'lyric?id=' + lrcid,
    //   success: function (res) {
    //     let lrc = parse_lrc(res.data.lrc && res.data.lrc.lyric ? res.data.lrc.lyric : '');
